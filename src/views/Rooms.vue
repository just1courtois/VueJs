...
let APIbuildings = 'http://justin-courtois.cleverapps.io/api/buildings'
let APIrooms = 'http://justin-courtois.cleverapps.io/api/rooms'


const requestOne = axios.get(APIrooms);
const requestTwo = axios.get(APIbuildings);
        ...
        methods: {
          ...
          createItem(){ //axios didn't work so I used
            var id = parseFloat(this.picked);
             fetch('http://justin-courtois.cleverapps.io/api/rooms/createByBuilding/'+id, {
                method: 'POST',
                headers: {
                    'accept': '*/*',
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({"current_temperature":0, "id":0, "name":"string","target_temperature":0 })
            })
             .then(
              Swal.fire({
                title: 'Created',
          ...
          renameItem(){
            ...
            axios.put('http://justin-courtois.cleverapps.io/api/rooms/rename/'+id+'/{name}?name='+name)
             .then(
              Swal.fire({
                title: 'Renamed',
                text: 'Press "Ok" to refresh',
